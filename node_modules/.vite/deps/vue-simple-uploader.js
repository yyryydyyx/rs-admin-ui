...
var C = { exports: {} };
var A = C.exports = (function e(t, i, s) {
  function r(o2, a) {
    if (!i[o2]) {
      if (!t[o2]) {
        if (!a && x) return x(o2);
        if (n) return n(o2, true);
  ...
  for (var n = x, o = 0; o < s.length; o++) r(s[o]);
  return r;
})({ 1: [function(e2, t, i) {
  var s = e2("./utils");
  function r(e3, t2, i2) {
  ...
  s.extend(r.prototype, { _event: function(e3, t2) {
    ...
    if (s.isFunction(e3)) switch (this.preprocessState) {
      case 0:
    ...
    function l(e4) {
      ...
      o.processingResponse = true, o.uploader.opts.processResponse(t3, (function(e5, t4) {
        if (o.processingResponse = false, o.xhr) {
          ...
          var i3 = o.status();
          if (i3 === n.SUCCESS || i3 === n.ERROR) o._event(i3, t4), i3 === n.ERROR && o.uploader.uploadNextChunk();
          else {
    ...
    if (1 === this.readState) return n.READING;
    if (this.pendingRetry || 1 === this.preprocessState) return n.UPLOADING;
    if (this.xhr) {
      if (this.xhr.readyState < 4 || this.processingResponse) return n.UPLOADING;
      var t2;
    ...
    } else a = new FormData(), s.each(n2, (function(e4, t3) {
      a.append(t3, e4);
  ...
  var s = e2("./utils").each, r = { _eventData: null, on: function(e3, t2) {
    ...
    this._eventData || (this._eventData = {}), this._eventData[e3] && this._eventData[e3].length && (t2 ? s(this._eventData[e3], (function(i2, s2) {
      if (i2 === t2) return this._eventData[e3].splice(s2, 1), false;
    }), this) : this._eventData[e3] = []);
  }, trigger: function(e3) {
    if (this._eventData || (this._eventData = {}), !this._eventData[e3]) return true;
  ...
  d.version = a, d.defaults = { chunkSize: 1048576, forceChunkSize: false, simultaneousUploads: 3, singleFile: false, fileParameterName: "file", progressCallbacksInterval: 500, speedSmoothingFactor: 0.1, query: {}, headers: {}, withCredentials: false, preprocess: null, method: "multipart", testMethod: "GET", uploadMethod: "POST", prioritizeFirstAndLastChunk: false, allowDuplicateUploads: false, target: "/", testChunks: true, generateUniqueIdentifier: null, maxChunkRetries: 0, chunkRetryInterval: null, permanentErrors: [404, 415, 500, 501], successStatuses: [200, 201, 202], onDropStopPropagation: false, initFileFn: null, readFileFn: f, checkChunkUploadedByResponse: null, initialPaused: false, processResponse: function(e3, t2) {
    ...
    for (var e3 = this.fileList.length - 1; e3 >= 0; e3--) this.fileList[e3].cancel();
    ...
    if (s.isFunction(t2)) return t2(e3);
    ...
    return s.each(this.files, (function(i2) {
      if (i2.uniqueIdentifier === e3) return t2 = i2, false;
    })), t2;
    ...
    if (this.opts.prioritizeFirstAndLastChunk && (s.each(this.files, (function(e4) {
      if (!e4.paused && (!r2 || e4._firstResponse || !e4.isUploading())) return e4.chunks.length && e4.chunks[0].status() === i2 ? (e4.chunks[0].send(), t2 = true, false) : e4.chunks.length > 1 && e4.chunks[e4.chunks.length - 1].status() === i2 ? (e4.chunks[e4.chunks.length - 1].send(), t2 = true, false) : void 0;
    })), t2)) return t2;
    if (s.each(this.files, (function(e4) {
      if (!e4.paused) {
        if (r2 && !e4._firstResponse && e4.isUploading()) return;
        s.each(e4.chunks, (function(e5) {
          if (e5.status() === i2) return e5.send(), t2 = true, false;
        }));
      }
      if (t2) return false;
    })), t2) return true;
    ...
    return s.each(this.files, (function(e4) {
      if (!e4.isComplete()) return n2 = true, false;
    ...
    return s.each(this.files, (function(n2) {
      return s.each(n2.chunks, (function(s2) {
        if (s2.status() === r2 && ++e3 >= i2) return t2 = false, false;
      })), t2;
    })), t2 && e3;
    ...
    function o2(e4) {
      e4.readEntries((function(t3) {
        t3.length ? (r2 += t3.length, s.each(t3, (function(e5) {
          ...
          } else e5.isDirectory && o2(e5.createReader());
        })), o2(e4)) : u2();
      }), l2);
    }
    function a2(e4, t3) {
  ...
  function o(e3) {
    ...
    if (i2.splice(s2 - 1, 1), s2--, i2.length) for (; r2 <= s2; ) t2.push(i2.slice(0, r2++).join("/") + "/");
    return t2;
  }
  s.extend(n.prototype, { _parseFile: function() {
    ...
    if (!this.isFolder) {
      ...
      for (var t2 = e3.forceChunkSize ? Math.ceil : Math.floor, i2 = Math.max(t2(this.size / e3.chunkSize), 1), n2 = 0; n2 < i2; n2++) this.chunks.push(new r(this.uploader, this, n2));
    }
    ...
    switch (t2) {
      ...
      case n2.ERROR:
        ...
        break;
      case n2.SUCCESS:
        if (this._updateUploadedChunks(i2, e3), this.error) return;
    ...
    for (var e3 = this.parent; e3 && e3 !== this.uploader; ) e3._errorFiles.push(this), e3.error = true, e3._errorFiles.length === e3.files.length && (e3.allError = true), e3 = e3.parent;
  }, _resetError: function() {
    this.error = this.allError = false;
    for (var e3 = this.parent, t2 = -1; e3 && e3 !== this.uploader; ) t2 = e3._errorFiles.indexOf(this), e3._errorFiles.splice(t2, 1), e3.allError = false, e3._errorFiles.length || (e3.error = false), e3 = e3.parent;
  }, isComplete: function() {
    if (!this.completed) {
      var e3 = false;
      this._eachAccess((function(t2) {
        if (!t2.isComplete()) return e3 = true, false;
      }), (function() {
        if (this.error) e3 = true;
        else {
          var t2 = r.STATUS;
          s.each(this.chunks, (function(i2) {
            var s2 = i2.status();
            if (s2 === t2.ERROR || s2 === t2.PENDING || s2 === t2.UPLOADING || s2 === t2.READING || 1 === i2.preprocessState || 1 === i2.readState) return e3 = true, false;
          }));
        }
      })), this.completed = !e3;
    }
    ...
    return this._eachAccess((function(t2) {
      if (t2.isUploading()) return e3 = true, false;
    }), (function() {
      var t2 = r.STATUS.UPLOADING;
      s.each(this.chunks, (function(i2) {
        if (i2.status() === t2) return e3 = true, false;
      }));
    })), e3;
  }, resume: function() {
    ...
    return this._eachAccess((function(s2, r2) {
      ...
      if (this.error) i2 = 1;
        ...
        if (1 === this.chunks.length) return this._prevProgress = Math.max(this._prevProgress, this.chunks[0].progress()), void (i2 = this._prevProgress);
        var e4 = 0;
    ...
    for (var e3 = this.parent; e3; ) {
      if (e3.parent === this.uploader) return e3;
      e3 = e3.parent;
    }
    return this;
  }, sizeUploaded: function() {
    ...
    return this._eachAccess((function(r2, n2) {
      ...
      if (this.paused || this.error) e3 = 0;
      else {
    ...
    if (e3.isFolder) for (; e3.files.length; ) {
    ...
    if (!e3.isFolder) {
      s.each(this.files, (function(t3, i3) {
        if (t3 === e3) return this.files.splice(i3, 1), false;
      }), this), e3.abort();
      for (var t2, i2 = e3.parent; i2 && i2 !== this; ) t2 = i2.parent, i2._removeFile(e3), i2 = t2;
    }
    e3.parent === this && s.each(this.fileList, (function(t3, i3) {
      if (t3 === e3) return this.fileList.splice(i3, 1), false;
  ...
  var s = Object.prototype, r = Array.prototype, n = s.toString, o = function(e3) {
    ...
    if (p.isDefined(e3.length)) for (var s2 = 0, r2 = e3.length; s2 < r2 && false !== t2.call(i2, e3[s2], s2, e3); s2++) ;
    else for (var n2 in e3) if (false === t2.call(i2, e3[n2], n2, e3)) break;
    ...
    for ("boolean" == typeof u2 && (f = u2, u2 = arguments[1] || {}, h++), "object" == typeof u2 || o(u2) || (u2 = {}), h === d && (u2 = this, h--); h < d; h++) if (null != (e3 = arguments[h])) for (t2 in e3) i2 = u2[t2], u2 !== (s2 = e3[t2]) && (f && s2 && (l(s2) || (r2 = a(s2))) ? (r2 ? (r2 = false, n2 = i2 && a(i2) ? i2 : []) : n2 = i2 && l(i2) ? i2 : {}, u2[t2] = p.extend(f, n2, s2)) : void 0 !== s2 && (u2[t2] = s2));
    return u2;
...
var U = { name: "uploader-btn", props: { directory: { type: Boolean, default: false }, single: { type: Boolean, default: false }, attrs: { type: Object, default: () => ({}) } }, setup(r) {
...
var w = { class: "uploader-btn", ref: "btn" };
...
var D = { name: "uploader-drop", setup() {
...
var L = { name: "uploader-file", props: { file: { type: Object, default: () => ({}) }, list: { type: Boolean, default: false } }, setup(t) {
  ...
  const o = ref(null), a = ref(false), l = ref(false), u = ref(0), p = ref(0), h = ref(false), d = ref(false), m = ref(0), S = ref(""), _ = ref(0), b = ref(0), k = ref(0), R = ref(""), F = ref(""), E = ref(""), x2 = computed((() => {
    ...
    const e2 = t.file;
    if (k.value === Number.POSITIVE_INFINITY || 0 === k.value) return "";
    let i = (function(e3) {
      const t2 = Math.floor(e3 / 31536e3);
      if (t2) return t2 + " year" + o2(t2);
      const i2 = Math.floor((e3 %= 31536e3) / 86400);
      if (i2) return i2 + " day" + o2(i2);
      const s3 = Math.floor((e3 %= 86400) / 3600);
      if (s3) return s3 + " hour" + o2(s3);
      const r2 = Math.floor((e3 %= 3600) / 60);
      if (r2) return r2 + " minute" + o2(r2);
      const n2 = e3 % 60;
    ...
    if (toRaw(t.file) === toRaw(o2)) {
      if (t.list && "fileSuccess" === e2) return void T2(i[2]);
      s.setupState[e2](...i);
    }
  }, L2 = () => {
  ...
  })), { response: o, paused: a, error: l, averageSpeed: u, currentSpeed: p, isComplete: h, isUploading: d, size: m, formatedSize: S, uploadedSize: _, progress: b, timeRemaining: k, type: R, extension: F, progressingClass: E, fileCategory: x2, progressStyle: C2, formatedAverageSpeed: P2, status: U2, statusText: w2, formatedTimeRemaining: D2, actionCheck: z2, pause: () => {
    t.file.pause(), z2(), L2();
  }, resume: () => {
    t.file.resume(), z2();
  }, remove: () => {
...
var O = ["status"];
var B = { class: "uploader-file-info" };
var G = { class: "uploader-file-name" };
var M = ["icon"];
var j = { class: "uploader-file-size" };
var q = createBaseVNode("div", { class: "uploader-file-meta" }, null, -1);
var $ = { class: "uploader-file-status" };
var H = { class: "uploader-file-actions" };
L.render = function(e2, t, i, s, u, f) {
  return openBlock(), createElementBlock("div", { class: "uploader-file", status: s.status }, [renderSlot(e2.$slots, "default", { file: i.file, list: i.list, status: s.status, paused: s.paused, error: s.error, response: s.response, averageSpeed: s.averageSpeed, formatedAverageSpeed: s.formatedAverageSpeed, currentSpeed: s.currentSpeed, isComplete: s.isComplete, isUploading: s.isUploading, size: s.size, formatedSize: s.formatedSize, uploadedSize: s.uploadedSize, progress: s.progress, progressStyle: s.progressStyle, progressingClass: s.progressingClass, timeRemaining: s.timeRemaining, formatedTimeRemaining: s.formatedTimeRemaining, type: s.type, extension: s.extension, fileCategory: s.fileCategory }, (() => [createBaseVNode("div", { class: normalizeClass(["uploader-file-progress", s.progressingClass]), style: normalizeStyle(s.progressStyle) }, null, 6), createBaseVNode("div", B, [createBaseVNode("div", G, [createBaseVNode("i", { class: "uploader-file-icon", icon: s.fileCategory }, null, 8, M), createTextVNode(toDisplayString(i.file.name), 1)]), createBaseVNode("div", j, toDisplayString(s.formatedSize), 1), q, createBaseVNode("div", $, [withDirectives(createBaseVNode("span", null, toDisplayString(s.statusText), 513), [[vShow, "uploading" !== s.status]]), withDirectives(createBaseVNode("span", null, [createBaseVNode("span", null, toDisplayString(s.progressStyle.progress) + " ", 1), createBaseVNode("em", null, toDisplayString(s.formatedAverageSpeed) + " ", 1), createBaseVNode("i", null, toDisplayString(s.formatedTimeRemaining), 1)], 512), [[vShow, "uploading" === s.status]])]), createBaseVNode("div", H, [createBaseVNode("span", { class: "uploader-file-pause", onClick: t[0] || (t[0] = (...e3) => s.pause && s.pause(...e3)) }), createBaseVNode("span", { class: "uploader-file-resume", onClick: t[1] || (t[1] = (...e3) => s.resume && s.resume(...e3)) }, "️"), createBaseVNode("span", { class: "uploader-file-retry", onClick: t[2] || (t[2] = (...e3) => s.retry && s.retry(...e3)) }), createBaseVNode("span", { class: "uploader-file-remove", onClick: t[3] || (t[3] = (...e3) => s.remove && s.remove(...e3)) })])])]))], 8, O);
};
var X = { name: "uploader-list", components: { UploaderFile: L }, setup() {
  const e2 = inject("uploader").proxy;
  return { fileList: computed((() => e2.fileList)) };
} };
var Y = { class: "uploader-list" };
...
var V = { name: "uploader-files", components: { UploaderFile: L }, setup() {
  const e2 = inject("uploader").proxy;
  return { files: computed((() => e2.files)) };
} };
var Z = { class: "uploader-files" };
...
var J = { name: "uploader", props: { options: { type: Object, default: () => ({}) }, autoStart: { type: Boolean, default: true }, fileStatusText: { type: [Object, Function], default: () => ({ success: "success", error: "error", uploading: "uploading", paused: "paused", waiting: "waiting" }) }, onFileAdded: Function, onFilesAdded: Function }, emits: ["change", "dragover", "dragenter", "dragleave", "file-success", "file-complete", "file-progress", "file-added", "files-added", "files-submitted", "file-removed", "file-retry", "file-error", "upload-start", "complete"], setup(t, { emit: i }) {
  const s = ref(false), r = ref([]), n = ref([]), o = getCurrentInstance();
  let a = new A(t.options);
  const l = (e2) => {
    const s2 = e2;
    if (t.onFileAdded) {
      if (false === t.onFileAdded(s2) || s2.ignored) return false;
    } else if (i(P("fileAdded"), s2), s2.ignored) return false;
  }, u = (e2, s2) => {
    if (t.onFilesAdded) {
      if (false === t.onFilesAdded(e2, s2) || e2.ignored || s2.ignored) return false;
    } else if (i(P("filesAdded"), e2, s2), e2.ignored || s2.ignored) return false;
  }, p = () => {
...
}, components: { UploaderBtn: U, UploaderDrop: D, UploaderUnsupport: z, UploaderList: X, UploaderFiles: V, UploaderFile: L } };
var K = { class: "uploader" };
var Q = createBaseVNode("p", null, "Drop files here to upload or", -1);
...
var W = { version: "1.0.3", install: function(e2, t) {
...
var vue_simple_uploader_es_default = W;
export {
